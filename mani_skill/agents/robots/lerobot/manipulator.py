...
class LeRobotRealAgent(BaseRealAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self._calibration_offset is None:
            logging.warning(
                "The calibration offset for the robot is not tuned!! Unless you are absolutely sure you don't need it, "
                "you will most likely get poor results for sim2real/real2sim transfer."
            )
    ...
    def set_target_qpos(self, qpos: Array):
        ...
        for motor_key, qpos_val in zip(self._motor_keys, qpos):
            ...
            if self._calibration_offset is not None and motor_key in self._calibration_offset:
                action[action_key] = (
